...
return val;
 ...
 initiateForAdmin()async{
  ...
  switchCategories(int selectedCategoryId) {
    ...
    print('________________________________________________$categorySelectedId');
    ...
    if (selectedCategoryId == 0) {
      ...
      if (productServiceSelected == 0) {
        productsAndServiceToBePassedInGridView = productList!
            .where((element) => element.category_id == selectedCategoryId.toString())
            .toList();
      } else {
    ...
    if (internetAvailable == true) {
      try {
        ...
        productListResponse = ProductsModel.fromMap(
            await PosRepository().fetchProductsAndService(type:'Product',category_id:categorySelectedId));
        notifyListeners();
    ...
    if (internetAvailable == true) {
      try {
        serviceListResponse = ProductsModel.fromMap(
            await PosRepository().fetchProductsAndService(type: 'Service', category_id: ''));
        notifyListeners();
